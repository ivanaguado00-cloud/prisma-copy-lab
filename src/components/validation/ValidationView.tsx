...
const VERDICT_META: Record<string, { label: string; bg: string; text: string; border: string; dot: string }> = {
  aprobada: {
    label: 'Aprobada',
    bg: '#052e16', text: '#10b981', border: '#065f46', dot: '#10b981',
  },
  aprobada_con_ajustes: {
    label: 'Aprobada con ajustes',
    bg: '#1c1400', text: '#f59e0b', border: '#92400e', dot: '#f59e0b',
  },
  no_aprobada: {
    label: 'No aprobada',
    bg: '#1c0000', text: '#ef4444', border: '#7f1d1d', dot: '#ef4444',
  },
}
...
function ValidationContent({ run }: { run: ValidationRunWithScores }) {
  const orderedScores = [...run.scores].sort((a, b) => a.criterionKey.localeCompare(b.criterionKey))
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        {run.suggestedRewrite && (
          <div className="rounded px-4 py-3 space-y-1.5" style={{ background: '#1c1400', border: '1px solid #92400e' }}>
            ...
            <p className="text-sm whitespace-pre-wrap leading-relaxed" style={{ color: '#fde68a' }}>
              {run.suggestedRewrite}
            </p>
          </div>
        )}
