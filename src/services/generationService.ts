...
import { formatProgramsForPrompt } from '../lib/briefingOptions'
...
// ── Bloque de variables CRM (compartido por todos los prompts) ───────────────

const CRM_VARIABLES_BLOCK = `VARIABLES DE PERSONALIZACIÓN CRM
...
a datos del destinatario o del programa. Tratalas como si fueran texto real.
...
- {{fecha_inicio}}         → fecha de inicio del programa (úsala solo si el
                             brief la menciona como dato relevante)
- {{condicion_comercial}}  → beneficio o condición comercial activa (úsala
                             solo si el brief la incluye)
...
export function buildUserPrompt(brief: Brief): string {
  ...
  const programsLabel = formatProgramsForPrompt(brief.programOrTitulation)

  return `Genera una propuesta de mensaje comercial con los siguientes datos:

- campaña: ${brief.title}
- programa o titulación: ${programsLabel}
...
const AB_VARIANT_STRATEGIES = {
  A: {
    label: 'Variante A — Enfoque en flexibilidad y progreso profesional',
    instruction: `VARIANTE A — ENFOQUE ARGUMENTAL: flexibilidad y progreso profesional.
Construye el mensaje enfatizando que la formación encaja en la vida del destinatario
(100% online, sin renunciar al trabajo) y que supone un avance real en su carrera.
La apertura debe crear identificación con la situación actual del destinatario.
El argumento central es: "puedes avanzar profesionalmente sin parar tu vida".`,
  },
  B: {
    label: 'Variante B — Enfoque en empleabilidad y retorno de la inversión',
    instruction: `VARIANTE B — ENFOQUE ARGUMENTAL: empleabilidad y retorno de la inversión.
Construye el mensaje enfatizando el impacto profesional concreto: salidas laborales,
mejora de posición, skills demandados por el mercado.
La apertura debe plantear un reto o necesidad del mercado que el programa resuelve.
El argumento central es: "el mercado necesita este perfil y esta formación te lo da".`,
  },
} as const

export type ABVariant = keyof typeof AB_VARIANT_STRATEGIES

function buildABUserPrompt(brief: Brief, variant: ABVariant): string {
  const strategy = AB_VARIANT_STRATEGIES[variant]
  const programsLabel = formatProgramsForPrompt(brief.programOrTitulation)
  const modeNote =
    brief.mode === 'exploracion'
      ? 'Modo: EXPLORACIÓN. Puedes proponer enfoques creativos o estructuras menos\nconvencionales, siempre dentro de las reglas de marca y sin romper tono.'
...
function buildABEmailUserPrompt(brief: Brief, variant: ABVariant): string {
  const strategy = AB_VARIANT_STRATEGIES[variant]
  const templateHint = brief.emailTemplate
    ? (EMAIL_TEMPLATE_HINTS[brief.emailTemplate] ?? '')
    : ''
...
Genera un email comercial con los siguientes datos:
...
export async function generateABVariant(brief: Brief, variant: ABVariant) {
  const client = getGenerationClient()
  const isEmail = brief.channel === 'email'

  const systemPrompt = isEmail ? EMAIL_GENERATION_SYSTEM_PROMPT : GENERATION_SYSTEM_PROMPT
  const userPrompt = isEmail
    ? buildABEmailUserPrompt(brief, variant)
    : buildABUserPrompt(brief, variant)
  ...
  const strategyLabel = AB_VARIANT_STRATEGIES[variant].label
